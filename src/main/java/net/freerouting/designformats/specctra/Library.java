...
import net.freerouting.geometry.planar.IntVector;
...
public class Library extends ScopeKeyword {
    ...
    public static void write_padstack_scope(WriteScopeParameter p_par, net.freerouting.library.Padstack p_padstack) throws java.io.IOException {
        ...
        for (int i = first_layer_no; i <= last_layer_no; ++i) {
            net.freerouting.geometry.planar.Shape curr_board_shape = p_padstack.get_shape(i);
            if (curr_board_shape == null) {
                continue;
            }
            net.freerouting.board.Layer board_layer = p_par.board.layer_structure.arr[i];
    ...
    static boolean read_padstack_scope(Scanner p_scanner, LayerStructure p_layer_structure,
                                       CoordinateTransform p_coordinate_transform, net.freerouting.library.Padstacks p_board_padstacks) {
        ...
        net.freerouting.geometry.planar.ConvexShape[] padstack_shapes = new net.freerouting.geometry.planar.ConvexShape[p_layer_structure.arr.length];
        Iterator<Shape> it = shape_list.iterator();
        while (it.hasNext()) {
            Shape pad_shape = it.next();
            net.freerouting.geometry.planar.Shape curr_shape = pad_shape.transform_to_board_rel(p_coordinate_transform);
            net.freerouting.geometry.planar.ConvexShape convex_shape;
            if (curr_shape instanceof net.freerouting.geometry.planar.ConvexShape) {
                convex_shape = (net.freerouting.geometry.planar.ConvexShape) curr_shape;
            } else {
                if (curr_shape instanceof PolygonShape) {
                    curr_shape = ((PolygonShape) curr_shape).convex_hull();
                }
                net.freerouting.geometry.planar.TileShape[] convex_shapes = curr_shape.split_to_convex();
            ...
            net.freerouting.geometry.planar.ConvexShape padstack_shape = convex_shape;
    ...
    public boolean read_scope(ReadScopeParameter p_par) {
        ...
        board.library.padstacks = new net.freerouting.library.Padstacks(p_par.board_handling.get_routing_board().layer_structure);
        ...
        if (p_par.via_padstack_names != null) {
            net.freerouting.library.Padstack[] via_padstacks = new net.freerouting.library.Padstack[p_par.via_padstack_names.size()];
            ...
            for (int i = 0; i < via_padstacks.length; ++i) {
                ...
                net.freerouting.library.Padstack curr_padstack = board.library.padstacks.get(curr_padstack_name);
            ...
            if (found_padstack_count != via_padstacks.length) {
                ...
                net.freerouting.library.Padstack[] corrected_padstacks = new net.freerouting.library.Padstack[found_padstack_count];
        ...
        board.library.packages = new net.freerouting.library.Packages(board.library.padstacks);
        Iterator<Package> it = package_list.iterator();
        while (it.hasNext()) {
            Package curr_package = it.next();
            net.freerouting.library.Package.Pin[] pin_arr = new net.freerouting.library.Package.Pin[curr_package.pin_info_arr.length];
            for (int i = 0; i < pin_arr.length; ++i) {
                ...
                net.freerouting.library.Padstack board_padstack = board.library.padstacks.get(pin_info.padstack_name);
                if (board_padstack == null) {
                    System.out.println("Library.read_scope: board padstack not found");
                    return false;
                }
                pin_arr[i] = new net.freerouting.library.Package.Pin(pin_info.pin_name, board_padstack.no, rel_coor, pin_info.rotation);
            }
            net.freerouting.geometry.planar.Shape[] outline_arr = new net.freerouting.geometry.planar.Shape[curr_package.outline.size()];
            ...
            net.freerouting.library.Package.Keepout[] keepout_arr = new net.freerouting.library.Package.Keepout[curr_package.keepouts.size()];
            ...
            for (int i = 0; i < keepout_arr.length; ++i) {
                ...
                net.freerouting.geometry.planar.Area curr_area = Shape.transform_area_to_board_rel(curr_keepout.shape_list, p_par.coordinate_transform);
                keepout_arr[i] = new net.freerouting.library.Package.Keepout(curr_keepout.area_name, curr_area, curr_layer.no);
            }
            net.freerouting.library.Package.Keepout[] via_keepout_arr = new net.freerouting.library.Package.Keepout[curr_package.via_keepouts.size()];
            it2 = curr_package.via_keepouts.iterator();
            for (int i = 0; i < via_keepout_arr.length; ++i) {
                Shape.ReadAreaScopeResult curr_keepout = it2.next();
                Layer curr_layer = (curr_keepout.shape_list.iterator().next()).layer;
                net.freerouting.geometry.planar.Area curr_area = Shape.transform_area_to_board_rel(curr_keepout.shape_list, p_par.coordinate_transform);
                via_keepout_arr[i] = new net.freerouting.library.Package.Keepout(curr_keepout.area_name, curr_area, curr_layer.no);
            }
            net.freerouting.library.Package.Keepout[] place_keepout_arr = new net.freerouting.library.Package.Keepout[curr_package.place_keepouts.size()];
            it2 = curr_package.place_keepouts.iterator();
            for (int i = 0; i < place_keepout_arr.length; ++i) {
                Shape.ReadAreaScopeResult curr_keepout = it2.next();
                Layer curr_layer = (curr_keepout.shape_list.iterator().next()).layer;
                net.freerouting.geometry.planar.Area curr_area = Shape.transform_area_to_board_rel(curr_keepout.shape_list, p_par.coordinate_transform);
                place_keepout_arr[i] = new net.freerouting.library.Package.Keepout(curr_keepout.area_name, curr_area, curr_layer.no);
            }
