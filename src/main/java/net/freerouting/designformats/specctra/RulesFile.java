...
public class RulesFile {
    ...
    public static void write(net.freerouting.interactive.BoardHandling p_board_handling, java.io.OutputStream p_output_stream, String p_design_name) {
    ...
    public static boolean read(java.io.InputStream p_input_stream, String p_design_name,
                               net.freerouting.interactive.BoardHandling p_board_handling) {
        BasicBoard routing_board = p_board_handling.get_routing_board();
        ...
        for (; ; ) {
            ...
            if (prev_token == Keyword.OPEN_BRACKET) {
                if (next_token == Keyword.RULE) {
                    ...
                    net.freerouting.board.AngleRestriction snap_angle = Structure.read_snap_angle(scanner);
                    if (snap_angle != null) {
                        routing_board.rules.set_trace_angle_restriction(snap_angle);
                    }
                } else if (next_token == Keyword.AUTOROUTE_SETTINGS) {
                    net.freerouting.interactive.AutorouteSettings autoroute_settings
    ...
    private static boolean read_via_info(Scanner p_scanner, BasicBoard p_board) {
        net.freerouting.rules.ViaInfo curr_via_info = Network.read_via_info(p_scanner, p_board);
        if (curr_via_info == null) {
            return false;
        }
        net.freerouting.rules.ViaInfo existing_via = p_board.rules.via_infos.get(curr_via_info.get_name());
