...
import net.freerouting.board.Layer;
...
public class WindowVia extends BoardSavableSubWindow
{
    ...
    {
        this.resources = java.util.ResourceBundle.getBundle("net.freerouting.gui.WindowVia", p_board_frame.get_locale());
    ...
    private class ShowPadstacksListener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            ...
            net.freerouting.library.BoardLibrary board_library = board_frame.board_panel.board_handling.get_routing_board().library;
            for (int i = 0; i < board_library.via_padstack_count(); ++i)
            {
                object_list.add( board_library.get_via_padstack(i));
            }
            net.freerouting.board.CoordinateTransform coordinate_transform = board_frame.board_panel.board_handling.coordinate_transform;
    ...
    private class AddPadstackListener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            net.freerouting.board.BasicBoard pcb = board_frame.board_panel.board_handling.get_routing_board();
            ...
            {
                Layer[] possible_start_layers = new  net.freerouting.board.Layer[pcb.layer_structure.arr.length - 1];
            ...
            if (!layers_selected)
            {
                ...
                Layer[] possible_end_layers = new  net.freerouting.board.Layer[pcb.layer_structure.arr.length - first_possible_end_layer_no];
            ...
            net.freerouting.geometry.planar.ConvexShape[] padstack_shapes = new net.freerouting.geometry.planar.ConvexShape[pcb.layer_structure.arr.length];
            net.freerouting.board.CoordinateTransform coordinate_transform = board_frame.board_panel.board_handling.coordinate_transform;
            ...
            for (int i = from_layer_no; i <= to_layer_no; ++i)
            {
                ...
                if (circle_radius > 0)
                {
                    padstack_shapes[i] = new  net.freerouting.geometry.planar.Circle(net.freerouting.geometry.planar.Point.ZERO, circle_radius);
                    shape_exists = true;
                }
            }
            if (!shape_exists)
            ...
            net.freerouting.library.Padstack new_padstack = pcb.library.padstacks.add(padstack_name, padstack_shapes, true, true);
    ...
    private class PadstackInputPanel extends javax.swing.JPanel
    {
        PadstackInputPanel(Layer p_from_layer, Layer p_to_layer, Double p_default_radius)
        {
            ...
            net.freerouting.board.LayerStructure layer_structure = board_frame.board_panel.board_handling.get_routing_board().layer_structure;
    ...
    private class RemovePadstackListener implements java.awt.event.ActionListener
    {
        ...
        {
            net.freerouting.board.BasicBoard pcb = board_frame.board_panel.board_handling.get_routing_board();
            net.freerouting.library.Padstack[] via_padstacks = pcb.library.get_via_padstacks();
            ...
            net.freerouting.library.Padstack selected_padstack = (net.freerouting.library.Padstack)selected_value;
            net.freerouting.rules.ViaInfo via_with_selected_padstack = null;
    ...
    private class ShowViasListener implements java.awt.event.ActionListener
    {
        ...
        {
            java.util.Collection<WindowObjectInfo.Printable> object_list = new java.util.LinkedList<WindowObjectInfo.Printable>();
            net.freerouting.rules.ViaInfos via_infos = board_frame.board_panel.board_handling.get_routing_board().rules.via_infos;
    ...
    private class EditViaRuleListener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            ...
            net.freerouting.rules.BoardRules board_rules = board_frame.board_panel.board_handling.get_routing_board().rules;
