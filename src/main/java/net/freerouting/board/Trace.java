...
public abstract class Trace extends Item implements Connectable, java.io.Serializable
{
    ...
    {
        return net.freerouting.boardgraphics.Drawable.MAX_DRAW_PRIORITY;
    }
    ...
    public boolean is_shove_fixed()
    {
        ...
        net.freerouting.rules.Nets nets = this.board.rules.nets;
        for (int curr_net_no : this.net_no_arr)
        {
            if (net.freerouting.rules.Nets.is_normal_net_no(curr_net_no))
            {
    ...
    public boolean is_cycle()
    {
        ...
        {
            net.freerouting.rules.Net curr_net = this.board.rules.nets.get(this.net_no_arr[0]);
