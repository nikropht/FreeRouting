...
public class WindowSnapshot extends BoardSavableSubWindow
{
    ...
    public WindowSnapshot(BoardFrame p_board_frame)
    {
        ...
        this.resources = java.util.ResourceBundle.getBundle("net.freerouting.gui.WindowSnapshot", p_board_frame.get_locale());
    ...
    void goto_selected()
    {
        ...
        if (index >= 0 && list_model.getSize() > index)
        {
            ...
            net.freerouting.interactive.SnapShot curr_snapshot =  (net.freerouting.interactive.SnapShot) list_model.elementAt(index);
            ...
            if (curr_snapshot.settings.get_snapshot_attributes().object_colors)
            {
                board_handling.graphics_context.item_color_table =
                        new net.freerouting.boardgraphics.ItemColorTableModel(curr_snapshot.graphics_context.item_color_table);
                board_handling.graphics_context.other_color_table =
                        new net.freerouting.boardgraphics.OtherColorTableModel(curr_snapshot.graphics_context.other_color_table);
            ...
            if (curr_snapshot.settings.get_snapshot_attributes().display_region)
            {
                ...
                {
                    board_handling.graphics_context.coordinate_transform = new net.freerouting.boardgraphics.CoordinateTransform(curr_snapshot.graphics_context.coordinate_transform);
    ...
    private class AddListener implements java.awt.event.ActionListener
    {
        ...
        {
            net.freerouting.interactive.SnapShot new_snapshot = net.freerouting.interactive.SnapShot.get_instance(name_field.getText(), board_frame.board_panel.board_handling);
