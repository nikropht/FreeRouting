...
public class WindowUnconnectedRoute extends WindowObjectListWithFilter {
    ...
    public WindowUnconnectedRoute(BoardFrame p_board_frame) {
        super(p_board_frame);
        this.resources = java.util.ResourceBundle.getBundle("net.freerouting.gui.CleanupWindows", p_board_frame.get_locale());
    ...
    protected void fill_list() {
        ...
        for (Item curr_item : board_items) {
            if (!(curr_item instanceof net.freerouting.board.Trace || curr_item instanceof net.freerouting.board.Via)) {
                continue;
            }
            ...
            for (Item curr_connnected_item : curr_connected_set) {
                ...
                if (!(curr_connnected_item instanceof net.freerouting.board.Trace || curr_connnected_item instanceof net.freerouting.board.Via)) {
            ...
            if (!terminal_item_found) {
                ...
                if (curr_item.net_count() == 1) {
                    net.freerouting.rules.Net curr_net = routing_board.rules.nets.get(curr_item.get_net_no(0));
                    if (curr_net != null) {
    ...
    protected void select_instances() {
        ...
        Set<net.freerouting.board.Item> selected_items = new java.util.TreeSet<net.freerouting.board.Item>();
    ...
    private class UnconnectedRouteInfo implements Comparable<UnconnectedRouteInfo> {
        private final net.freerouting.rules.Net net;
        private final Collection<Item> item_list;
        private final int id_no;
        private final Integer trace_count;
        private final Integer via_count;
        public UnconnectedRouteInfo(net.freerouting.rules.Net p_net, Collection<Item> p_item_list) {
            this.net = p_net;
            ...
            for (Item curr_item : p_item_list) {
                if (curr_item instanceof net.freerouting.board.Trace) {
                    ++curr_trace_count;
                } else if (curr_item instanceof net.freerouting.board.Via) {
                    ++curr_via_count;
                }
            }
