...
public class WindowRouteParameter extends BoardSavableSubWindow {
    ...
    public WindowRouteParameter(BoardFrame p_board_frame) {
        ...
        java.util.ResourceBundle resources =
                java.util.ResourceBundle.getBundle("net.freerouting.gui.WindowRouteParameter", p_board_frame.get_locale());
        ...
        if (this.board_handling.get_routing_board().get_test_level() != net.freerouting.board.TestLevel.RELEASE_VERSION) {
     ...
     */
    public void refresh() {
        net.freerouting.board.AngleRestriction snap_angle = this.board_handling.get_routing_board().rules.get_trace_angle_restriction();

        if (snap_angle == net.freerouting.board.AngleRestriction.NINETY_DEGREE) {
            snap_angle_90_button.setSelected(true);
        } else if (snap_angle == net.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE) {
    ...
    private class SnapAngle90Listener implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == net.freerouting.board.AngleRestriction.NINETY_DEGREE) {
                return;
            }
            Collection<net.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            boolean free_angle_traces_found = false;
            for (net.freerouting.board.Trace curr_trace : trace_list) {
                if (curr_trace instanceof net.freerouting.board.PolylineTrace) {
                    if (!((net.freerouting.board.PolylineTrace) curr_trace).polyline().is_orthogonal()) {
            ...
            if (free_angle_traces_found) {
                java.util.ResourceBundle resources =
                        java.util.ResourceBundle.getBundle("net.freerouting.gui.WindowRouteParameter", current_locale);
            ...
            board_handling.set_current_snap_angle(net.freerouting.board.AngleRestriction.NINETY_DEGREE);
        }
    }

    private class SnapAngle45Listener implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == net.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE) {
                return;
            }
            Collection<net.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            boolean free_angle_traces_found = false;
            for (net.freerouting.board.Trace curr_trace : trace_list) {
                if (curr_trace instanceof net.freerouting.board.PolylineTrace) {
                    if (!((net.freerouting.board.PolylineTrace) curr_trace).polyline().is_multiple_of_45_degree()) {
                        free_angle_traces_found = true;
                        break;
                    }
                }
            ...
            board_handling.set_current_snap_angle(net.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE);
        }
    }

    private class SnapAngleNoneListener implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            board_handling.set_current_snap_angle(net.freerouting.board.AngleRestriction.NONE);
        }
    }
    ...
    private class RestrictPinExitDirectionsListener implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            if (restrict_pin_exit_directions_check_box.isSelected()) {
                net.freerouting.rules.BoardRules board_rules = board_handling.get_routing_board().rules;
