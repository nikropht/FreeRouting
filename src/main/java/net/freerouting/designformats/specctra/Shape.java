...
public abstract class Shape
{
     ...
     */
    public static net.freerouting.geometry.planar.Area transform_area_to_board(Collection<Shape> p_area, CoordinateTransform p_coordinate_transform)
    {
        ...
        net.freerouting.geometry.planar.Shape boundary_shape = boundary.transform_to_board(p_coordinate_transform);
        net.freerouting.geometry.planar.Area result;
        if (hole_count == 0)
        {
        ...
        {
            // Area with holes
            if (!(boundary_shape instanceof net.freerouting.geometry.planar.PolylineShape))
            {
            ...
            {
                net.freerouting.geometry.planar.Shape hole_shape = it.next().transform_to_board(p_coordinate_transform);
     ...
     * the other shapes are holes of the area.
     */
    public static net.freerouting.geometry.planar.Area transform_area_to_board_rel(Collection<Shape> p_area, CoordinateTransform p_coordinate_transform)
    {
        int hole_count = p_area.size() - 1;
        if (hole_count <= -1)
        {
        ...
        Iterator<Shape> it = p_area.iterator();
        Shape boundary = it.next();
        net.freerouting.geometry.planar.Shape boundary_shape = boundary.transform_to_board_rel(p_coordinate_transform);
        net.freerouting.geometry.planar.Area result;
        if (hole_count == 0)
        {
            result = boundary_shape;
        ...
        else
        {
            ...
            for (int i = 0; i < holes.length; ++i)
            {
                net.freerouting.geometry.planar.Shape hole_shape = it.next().transform_to_board_rel(p_coordinate_transform);
                if (!(hole_shape instanceof PolylineShape))
                {
                    System.out.println("Shape.transform_area_to_board: PolylineShape expected");
                    return null;
     ...
     */
    public abstract net.freerouting.geometry.planar.Shape transform_to_board(CoordinateTransform p_coordinate_transform);
     ...
     */
    public abstract net.freerouting.geometry.planar.Shape transform_to_board_rel(CoordinateTransform p_coordinate_transform);
