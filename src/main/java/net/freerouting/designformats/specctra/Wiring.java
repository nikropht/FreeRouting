...
import net.freerouting.board.Via;
import net.freerouting.board.Trace;
import net.freerouting.board.PolylineTrace;
import net.freerouting.board.FixedState;
import net.freerouting.board.ItemSelectionFilter;
...
class Wiring extends ScopeKeyword
{
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        for(;;)
        {
            ...
            write_conduction_area_scope(p_par, (net.freerouting.board.ConductionArea) curr_ob);
        }
    ...
    private static void write_wire_scope(WriteScopeParameter p_par, Trace p_wire) throws java.io.IOException
    {
        ...
        net.freerouting.rules.Net wire_net = null;
    ...
    private static void write_conduction_area_scope(WriteScopeParameter p_par, net.freerouting.board.ConductionArea  p_conduction_area) throws java.io.IOException
    {
        ...
        net.freerouting.rules.Net curr_net = p_par.board.rules.nets.get(p_conduction_area.get_net_no(0));
        net.freerouting.geometry.planar.Area curr_area = p_conduction_area.get_area();
        ...
        if (curr_area instanceof net.freerouting.geometry.planar.Shape)
        {
            boundary_shape = (net.freerouting.geometry.planar.Shape) curr_area;
    ...
    static private void write_net(net.freerouting.rules.Net p_net, IndentFileWriter p_file, IdentifierType p_identifier_type) throws java.io.IOException
    {
        p_file.new_line();
        p_file.write("(");
    ...
    private Item read_wire_scope(ReadScopeParameter p_par)
    {
        ...
        net.freerouting.board.FixedState fixed = net.freerouting.board.FixedState.UNFIXED;
        ...
        net.freerouting.rules.NetClass net_class = board.rules.get_default_net_class();
        Collection<net.freerouting.rules.Net> found_nets = get_subnets(net_id, board.rules);
        int[] net_no_arr = new int[found_nets.size()];
        int curr_index = 0;
        for (net.freerouting.rules.Net curr_net : found_nets)
        {
        ...
        if (border_shape != null)
        {
            ...
            {
                clearance_class_no =
                        net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
            }
        ...
        else if (path instanceof PolygonPath)
        {
            ...
            {
                clearance_class_no =
                        net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.TRACE);
            }
    ...
    private static Collection<net.freerouting.rules.Net> get_subnets(Net.Id p_net_id, net.freerouting.rules.BoardRules p_rules)
    {
        Collection<net.freerouting.rules.Net> found_nets = new LinkedList<net.freerouting.rules.Net>();
        if (p_net_id != null)
        {
            if (p_net_id.subnet_number > 0)
            {
                net.freerouting.rules.Net found_net = p_rules.nets.get(p_net_id.name, p_net_id.subnet_number);
                if (found_net != null)
                {
    ...
    private boolean read_via_scope(ReadScopeParameter p_par)
    {
        try
        {
            ...
            RoutingBoard board = p_par.board_handling.get_routing_board();
            net.freerouting.library.Padstack curr_padstack = board.library.padstacks.get(padstack_name);
            ...
            if (clearance_class_no < 0)
            {
                clearance_class_no =  net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
            }
    ...
    private static boolean via_exists(IntPoint p_location, net.freerouting.library.Padstack p_padstack,
            int[] p_net_no_arr, net.freerouting.board.BasicBoard p_board)
    {
    ...
    static net.freerouting.board.FixedState calc_fixed(Scanner p_scanner)
    {
        try
        {
            net.freerouting.board.FixedState result = net.freerouting.board.FixedState.UNFIXED;
            Object next_token = p_scanner.next_token();
            if (next_token == Keyword.SHOVE_FIXED)
            {
                result = net.freerouting.board.FixedState.SHOVE_FIXED;
            }
            else if (next_token == Keyword.FIX)
            {
                result = net.freerouting.board.FixedState.SYSTEM_FIXED;
            }
            else if (next_token != Keyword.NORMAL)
            {
                result = net.freerouting.board.FixedState.USER_FIXED;
            }
            next_token = p_scanner.next_token();
            if (next_token != Keyword.CLOSED_BRACKET)
            {
                System.out.println("Wiring.is_fixed: ) expected");
                return net.freerouting.board.FixedState.UNFIXED;
            }
            return result;
        }
        catch (java.io.IOException e)
