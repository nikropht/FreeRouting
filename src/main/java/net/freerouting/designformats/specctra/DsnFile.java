...
public class DsnFile {
     ...
     */
    public static ReadResult read(java.io.InputStream p_input_stream, net.freerouting.interactive.BoardHandling p_board_handling,
                                  net.freerouting.board.BoardObservers p_observers, net.freerouting.datastructures.IdNoGenerator p_item_id_no_generator, TestLevel p_test_level) {
     ...
     */
    private static boolean adjust_plane_autoroute_settings(net.freerouting.interactive.BoardHandling p_board_handling) {
        BasicBoard routing_board = p_board_handling.get_routing_board();
        net.freerouting.board.LayerStructure board_layer_structure = routing_board.layer_structure;
        if (board_layer_structure.arr.length <= 2) {
            return false;
        }
        for (net.freerouting.board.Layer curr_layer : board_layer_structure.arr) {
        ...
        java.util.Collection<net.freerouting.board.ConductionArea> conduction_area_list = new java.util.LinkedList<net.freerouting.board.ConductionArea>();
        java.util.Collection<net.freerouting.board.Item> item_list = routing_board.get_items();
        for (net.freerouting.board.Item curr_item : item_list) {
            if (curr_item instanceof net.freerouting.board.Trace) {
                int curr_layer = ((net.freerouting.board.Trace) curr_item).get_layer();
                layer_contains_wires_arr[curr_layer] = true;
            } else if (curr_item instanceof net.freerouting.board.ConductionArea) {
                conduction_area_list.add((net.freerouting.board.ConductionArea) curr_item);
            }
        }
        ...
        for (int i = 0; i < board_outline.shape_count(); ++i) {
            net.freerouting.geometry.planar.TileShape[] curr_piece_arr = board_outline.get_shape(i).split_to_convex();
            if (curr_piece_arr != null) {
                for (net.freerouting.geometry.planar.TileShape curr_piece : curr_piece_arr) {
                    board_area += curr_piece.area();
                }
            }
        }
        for (net.freerouting.board.ConductionArea curr_conduction_area : conduction_area_list) {
            int layer_no = curr_conduction_area.get_layer();
            if (layer_contains_wires_arr[layer_no]) {
                continue;
            }
            net.freerouting.board.Layer curr_layer = routing_board.layer_structure.arr[layer_no];
            if (!curr_layer.is_signal || layer_no == 0 || layer_no == board_layer_structure.arr.length - 1) {
                continue;
            }
            net.freerouting.geometry.planar.TileShape[] convex_pieces = curr_conduction_area.get_area().split_to_convex();
            double curr_area = 0;
            for (net.freerouting.geometry.planar.TileShape curr_piece : convex_pieces) {
            ...
            for (int i = 0; i < curr_conduction_area.net_count(); ++i) {
                net.freerouting.rules.Net curr_net = routing_board.rules.nets.get(curr_conduction_area.get_net_no(i));
                curr_net.set_contains_plane(true);
                nothing_changed = false;
            }

            changed_layer_arr[layer_no] = true;
            if (curr_conduction_area.get_fixed_state().ordinal() < net.freerouting.board.FixedState.USER_FIXED.ordinal()) {
                curr_conduction_area.set_fixed_state(net.freerouting.board.FixedState.USER_FIXED);
            }
        }
        if (nothing_changed) {
            return false;
     ...
     */
    public static boolean write(net.freerouting.interactive.BoardHandling p_board_handling, java.io.OutputStream p_file, String p_design_name, boolean p_compat_mode) {
    ...
    private static void write_pcb_scope(net.freerouting.interactive.BoardHandling p_board_handling, IndentFileWriter p_file, String p_design_name, boolean p_compat_mode)
