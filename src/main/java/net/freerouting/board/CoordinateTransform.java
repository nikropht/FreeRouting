...
public class CoordinateTransform implements java.io.Serializable {
    ...
    public PrintableShape board_to_user(net.freerouting.geometry.planar.Shape p_shape, java.util.Locale p_locale) {
        PrintableShape result;
        if (p_shape instanceof net.freerouting.geometry.planar.Circle) {
            result = board_to_user((net.freerouting.geometry.planar.Circle) p_shape, p_locale);
        } else if (p_shape instanceof net.freerouting.geometry.planar.IntBox) {
            result = board_to_user((net.freerouting.geometry.planar.IntBox) p_shape, p_locale);
        } else if (p_shape instanceof net.freerouting.geometry.planar.PolylineShape) {
            result = board_to_user((net.freerouting.geometry.planar.PolylineShape) p_shape, p_locale);
        } else {
    ...
    public PrintableShape.Circle board_to_user(net.freerouting.geometry.planar.Circle p_circle, java.util.Locale p_locale) {
        return new PrintableShape.Circle(board_to_user(p_circle.center.to_float()),
                board_to_user(p_circle.radius), p_locale);
    }

    public PrintableShape.Rectangle board_to_user(net.freerouting.geometry.planar.IntBox p_box, java.util.Locale p_locale) {
        return new PrintableShape.Rectangle(board_to_user(p_box.ll.to_float()),
                board_to_user(p_box.ur.to_float()), p_locale);
    }

    public PrintableShape.Polygon board_to_user(net.freerouting.geometry.planar.PolylineShape p_shape, java.util.Locale p_locale) {
