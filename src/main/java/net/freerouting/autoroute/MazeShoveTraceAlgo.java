...
import net.freerouting.geometry.planar.Direction;
import net.freerouting.geometry.planar.LineSegment;

import net.freerouting.board.Item;
import net.freerouting.board.RoutingBoard;
import net.freerouting.board.ShoveTraceAlgo;
...
public class MazeShoveTraceAlgo
{
            ...
            boolean p_shove_to_the_left, Collection<DoorSection> p_to_door_list)
    {
        ...
        if (!(p_obstacle_room.get_item() instanceof net.freerouting.board.PolylineTrace))
        {
            return true;
        }
        net.freerouting.board.PolylineTrace obstacle_trace = (net.freerouting.board.PolylineTrace)p_obstacle_room.get_item();
        ...
        for (ExpansionDoor curr_door : room_doors)
        {
            ...
            else if (!segment_ist_point)
            {
                ...
                {
                    if (p_board.get_test_level() == net.freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT)
                    {
     ...
     */
    private static boolean end_points_matching(net.freerouting.board.PolylineTrace p_trace, Item p_from_item)
    {
        ...
        if (p_from_item instanceof net.freerouting.board.DrillItem)
        {
            Point from_center = ((net.freerouting.board.DrillItem) p_from_item).get_center();
            points_matching =  from_center.equals(p_trace.first_corner()) || from_center.equals(p_trace.last_corner());
        }
        else if (p_from_item instanceof net.freerouting.board.PolylineTrace)
        {
            net.freerouting.board.PolylineTrace from_trace = (net.freerouting.board.PolylineTrace) p_from_item;
