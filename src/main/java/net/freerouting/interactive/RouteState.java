...
public class RouteState extends InteractiveState
{
    ...
    public static RouteState get_instance(FloatPoint p_location, InteractiveState p_parent_state, BoardHandling p_board_handling, Logfile p_logfile)
    {
        ...
        if (picked_item instanceof net.freerouting.board.Pin && net_count > 1)
        {
            // tie pin, remove nets, which are already conneccted to this pin on the current layer.
            route_net_no_arr = get_route_net_numbers_at_tie_pin((net.freerouting.board.Pin) picked_item, p_board_handling.settings.layer);
        }
        else
        {
        ...
        net.freerouting.board.RoutingBoard routing_board = p_board_handling.get_routing_board();
        ...
        net.freerouting.rules.Net curr_net = routing_board.rules.nets.get(route_net_no_arr[0]);
        if (curr_net == null)
        {
            return null;
        }
    ...
    {
        net.freerouting.board.RoutingBoard routing_board = p_hdlg.get_routing_board();
    ...
    public InteractiveState key_typed(char p_key_char)
    {
        ...
        if (Character.isDigit(p_key_char))
        {
            ...
            net.freerouting.board.LayerStructure layer_structure = hdlg.get_routing_board().layer_structure;
            ...
            d = Math.max(d - 1, 0);
            net.freerouting.board.Layer new_layer = layer_structure.get_signal_layer(d);
    ...
    public void display_default_message()
    {
        if (route != null)
        {
            net.freerouting.rules.Net curr_net = hdlg.get_routing_board().rules.nets.get(route.net_no_arr[0]);
