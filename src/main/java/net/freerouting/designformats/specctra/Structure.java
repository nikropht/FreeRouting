...
import net.freerouting.geometry.planar.PolylineShape;
...
import net.freerouting.datastructures.UndoableObjects.Storable;

import net.freerouting.board.FixedState;
...
class Structure extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        for (;;)
        {
            ...
            if (prev_token == OPEN_BRACKET)
            {
                ...
                else if (next_token == Keyword.SNAP_ANGLE)
                {
                    ...
                    net.freerouting.board.AngleRestriction snap_angle = read_snap_angle(p_par.scanner);
                    if (snap_angle != null)
                    {
        ...
        net.freerouting.board.RoutingBoard board = p_par.board_handling.get_routing_board();
        if (board == null)
        {
            return false;
        }
        ...
        while (it.hasNext())
        {
            ...
            net.freerouting.rules.Net curr_net = board.rules.nets.get(plane_info.net_name, 1);
            if (curr_net == null)
            {
                System.out.println("Plane.read_scope: net not found");
                continue;
            }
            net.freerouting.geometry.planar.Area plane_area =
            ...
            if (curr_layer.no >= 0)
            {
                ...
                {
                    clearance_class_no = curr_net.get_class().default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
                }
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        for (;;)
        {
            ...
            if (curr_ob instanceof net.freerouting.board.BoardOutline)
            {
                break;
            }
        }
        ...
        net.freerouting.board.BoardOutline outline = (net.freerouting.board.BoardOutline) curr_ob;
        ...
        for (;;)
        {
            ...
            if (!(curr_ob instanceof net.freerouting.board.ObstacleArea))
            {
                continue;
            }
            net.freerouting.board.ObstacleArea curr_keepout = (net.freerouting.board.ObstacleArea) curr_ob;
        ...
        for (;;)
        {
            ...
            net.freerouting.board.ConductionArea curr_area = (net.freerouting.board.ConductionArea) curr_ob;
            if (p_par.board.layer_structure.arr[curr_area.get_layer()].is_signal)
            {
                // These conduction areas are written in the wiring scope.
                continue;
            }
            Plane.write_scope(p_par, (net.freerouting.board.ConductionArea) curr_ob);
        }
        p_par.file.end_scope();
    }
    ...
    private static void write_via_padstacks(net.freerouting.library.BoardLibrary p_library, IndentFileWriter p_file,
            IdentifierType p_identifier_type) throws java.io.IOException
    {
        ...
        {
            net.freerouting.library.Padstack curr_padstack = p_library.get_via_padstack(i);
    ...
    private static void write_control_scope(net.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file) throws java.io.IOException
    {
        p_file.start_scope();
    ...
    private static void write_keepout_scope(WriteScopeParameter p_par, net.freerouting.board.ObstacleArea p_keepout) throws java.io.IOException
    {
        net.freerouting.geometry.planar.Area keepout_area = p_keepout.get_area();
        int layer_no = p_keepout.get_layer();
        net.freerouting.board.Layer board_layer = p_par.board.layer_structure.arr[layer_no];
        Layer keepout_layer = new Layer(board_layer.name, layer_no, board_layer.is_signal);
        net.freerouting.geometry.planar.Shape boundary_shape;
        net.freerouting.geometry.planar.Shape[] holes;
        if (keepout_area instanceof net.freerouting.geometry.planar.Shape)
        {
            boundary_shape = (net.freerouting.geometry.planar.Shape) keepout_area;
            holes = new net.freerouting.geometry.planar.Shape[0];
        }
        else
        {
        ...
        p_par.file.start_scope();
        if (p_keepout instanceof net.freerouting.board.ViaObstacleArea)
        {
    ...
    static net.freerouting.board.AngleRestriction read_snap_angle(Scanner p_scanner)
    {
        try
        {
            Object next_token = p_scanner.next_token();
            net.freerouting.board.AngleRestriction snap_angle;
            if (next_token == Keyword.NINETY_DEGREE)
            {
                snap_angle = net.freerouting.board.AngleRestriction.NINETY_DEGREE;
            }
            else if (next_token == Keyword.FORTYFIVE_DEGREE)
            {
                snap_angle = net.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE;
            }
            else if (next_token == Keyword.NONE)
            {
                snap_angle = net.freerouting.board.AngleRestriction.NONE;
            }
            else
            {
    ...
    static void write_snap_angle(IndentFileWriter p_file, net.freerouting.board.AngleRestriction p_angle_restriction) throws java.io.IOException
    {
        p_file.start_scope();
        p_file.write("snap_angle ");
        p_file.new_line();

        if (p_angle_restriction == net.freerouting.board.AngleRestriction.NINETY_DEGREE)
        {
            p_file.write("ninety_degree");
        }
        else if (p_angle_restriction == net.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
        {
    ...
    private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info)
    {
        ...
        for (int i = 0; i < layer_count; ++i)
        {
            ...
            board_layer_arr[i] = new net.freerouting.board.Layer(curr_layer.name, curr_layer.is_signal);
        }
        net.freerouting.board.LayerStructure board_layer_structure = new net.freerouting.board.LayerStructure(board_layer_arr);
        ...
        net.freerouting.rules.ClearanceMatrix clearance_matrix = net.freerouting.rules.ClearanceMatrix.get_default_instance(board_layer_structure, 0);
        net.freerouting.rules.BoardRules board_rules = new net.freerouting.rules.BoardRules(board_layer_structure, clearance_matrix);
        net.freerouting.board.Communication.SpecctraParserInfo specctra_parser_info =
                new net.freerouting.board.Communication.SpecctraParserInfo(p_par.string_quote, p_par.host_cad,
                p_par.host_version, p_par.constants, p_par.write_resolution, p_par.dsn_file_generated_by_host);
        net.freerouting.board.Communication board_communication =
                new net.freerouting.board.Communication(p_par.unit, p_par.resolution, specctra_parser_info,
        ...
        net.freerouting.board.BasicBoard board = p_par.board_handling.get_routing_board();
    ...
    private static void insert_missing_power_planes(Collection<Layer> p_layer_info,
            NetList p_netlist, net.freerouting.board.BasicBoard p_board)
    {
        Collection<net.freerouting.board.ConductionArea> conduction_areas = p_board.get_conduction_areas();
        for (Layer curr_layer : p_layer_info)
        {
            ...
            for (net.freerouting.board.ConductionArea curr_conduction_area : conduction_areas)
            {
            ...
            if (!conduction_area_found && !curr_layer.net_names.isEmpty())
            {
                ...
                net.freerouting.rules.Net curr_net = p_board.rules.nets.get(curr_net_id.name, curr_net_id.subnet_number);
                {
                    if (curr_net == null)
                    {
    ...
    static private int append_clearance_class(BoardRules p_board_rules, String p_name)
    {
        ...
        net.freerouting.rules.NetClass default_net_class = p_board_rules.get_default_net_class();
     ...
     */
    private static boolean clearance_equals(net.freerouting.rules.ClearanceMatrix p_cl_matrix, int p_layer_1, int p_layer_2)
    {
    ...
    {
        net.freerouting.geometry.planar.Area keepout_area =
            ...
            String p_clearance_class_name, KeepoutType p_keepout_type, FixedState p_fixed_state)
    {
        ...
        {
            clearance_class_no =
                    p_board.rules.get_default_net_class().default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
        }
        else
        {
