...
import net.freerouting.geometry.planar.Vector;
...
import net.freerouting.rules.DefaultItemClearanceClasses.ItemClass;
...
public class Network extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        Collection<net.freerouting.rules.ViaInfo> via_infos = new LinkedList<net.freerouting.rules.ViaInfo>();
        ...
        for (;;)
        {
            ...
            if (prev_token == OPEN_BRACKET)
            {
                ...
                {
                    net.freerouting.rules.ViaInfo curr_via_info = read_via_info(p_par.scanner, p_par.board_handling.get_routing_board());
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        Collection<net.freerouting.board.Pin> board_pins = p_par.board.get_pins();
    ...
    public static void write_via_infos(net.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
    ...
    public static void write_via_rules(net.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
            throws java.io.IOException
    {
        for (net.freerouting.rules.ViaRule curr_rule : p_rules.via_rules)
        {
            p_file.start_scope();
    ...
    public static void write_net_class(net.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
        p_par.file.start_scope();
    ...
    private static void write_circuit(net.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
            ...
            CoordinateTransform p_coordinate_transform, LayerStructure p_layer_structure, java.util.Locale p_locale)
    {
        ...
        for (Collection<Net.Pin> curr_pin_list : subnet_pin_lists)
        {
            ...
            if (!net_rules.isEmpty())
            {
                ...
                net.freerouting.rules.Net board_net = p_board.rules.nets.get(curr_subnet.id.name, curr_subnet.id.subnet_number);
                if (board_net == null)
                {
                ...
                while (it.hasNext())
                {
                    ...
                    if (curr_ob instanceof Rule.WidthRule)
                    {
                        ...
                        net.freerouting.rules.NetClass net_rule =
    ...
    static net.freerouting.rules.ViaInfo read_via_info(Scanner p_scanner, net.freerouting.board.BasicBoard p_board)
    {
        try
        {
            p_scanner.yybegin(SpecctraFileScanner.NAME);
            ...
            net.freerouting.library.Padstack via_padstack = p_board.library.get_via_padstack(padstack_name);
            ...
            return new net.freerouting.rules.ViaInfo(name, via_padstack, clearance_class, attach_allowed, p_board.rules);
        } catch (java.io.IOException e)
        {
    ...
    static Collection<String> read_via_rule(Scanner p_scanner, net.freerouting.board.BasicBoard p_board)
    {
        try
        {
    ...
    private static void insert_via_infos(Collection<net.freerouting.rules.ViaInfo> p_via_infos, RoutingBoard p_board, boolean p_attach_allowed)
    {
        if (p_via_infos.size() > 0)
        {
            for (net.freerouting.rules.ViaInfo curr_info : p_via_infos)
            {
    ...
    private static void create_default_via_infos(net.freerouting.board.BasicBoard p_board, net.freerouting.rules.NetClass p_net_class, boolean p_attach_allowed)
    {
        int cl_class = p_net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
        ...
        {
            net.freerouting.library.Padstack curr_padstack = p_board.library.get_via_padstack(i);
            ...
            net.freerouting.rules.ViaInfo found_via_info =
                    new net.freerouting.rules.ViaInfo(via_name, curr_padstack, cl_class, attach_allowed, p_board.rules);
            p_board.rules.via_infos.add(found_via_info);
        }
    }

    private static void insert_via_rules(Collection<Collection<String>> p_via_rules, net.freerouting.board.BasicBoard p_board)
    {
     ...
     */
    static boolean add_via_rule(Collection<String> p_name_list, net.freerouting.board.BasicBoard p_board)
    {
        Iterator<String> it = p_name_list.iterator();
        String rule_name = it.next();
        net.freerouting.rules.ViaRule existing_rule = p_board.rules.get_via_rule(rule_name);
        net.freerouting.rules.ViaRule curr_rule = new net.freerouting.rules.ViaRule(rule_name);
        boolean rule_ok = true;
        while (it.hasNext())
        {
            net.freerouting.rules.ViaInfo curr_via = p_board.rules.via_infos.get(it.next());
            if (curr_via != null)
            {
    ...
    {
        net.freerouting.board.BasicBoard routing_board = p_par.board_handling.get_routing_board();
    ...
    static void insert_net_class(NetClass p_class, LayerStructure p_layer_structure, net.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform,
            boolean p_via_at_smd_allowed)
    {
        net.freerouting.rules.NetClass board_net_class = p_board.rules.append_net_class(p_class.name);
        ...
        {
            net.freerouting.rules.ViaRule via_rule = p_board.rules.get_via_rule(p_class.via_rule);
            if (via_rule != null)
            {
        ...
        {
            Collection<net.freerouting.rules.Net> curr_net_list = p_board.rules.nets.get(curr_net_name);
            for (net.freerouting.rules.Net curr_net : curr_net_list)
            {
    ...
    static private void insert_class_pairs(Collection<NetClass.ClassClass> p_class_classes, ReadScopeParameter p_par)
    {
        for (NetClass.ClassClass curr_class_class : p_class_classes)
        {
            ...
            while (it1.hasNext())
            {
                ...
                net.freerouting.rules.NetClass first_class = routing_board.rules.net_classes.get(first_name);
                if (first_class == null)
                {
                ...
                else
                {
                    ...
                    while (it2.hasNext())
                    {
                        ...
                        net.freerouting.rules.NetClass second_class = routing_board.rules.net_classes.get(second_name);
    ...
    static private void insert_class_pair_info(NetClass.ClassClass p_class_class, net.freerouting.rules.NetClass p_first_class, net.freerouting.rules.NetClass p_second_class,
            net.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform)
    {
    ...
    static private void add_mixed_clearance_rule(net.freerouting.rules.ClearanceMatrix p_clearance_matrix, net.freerouting.rules.NetClass p_first_class,
            net.freerouting.rules.NetClass p_second_class, Rule.ClearanceRule p_clearance_rule, int p_layer_no,
    ...
    static private void create_default_clearance_classes(net.freerouting.rules.NetClass p_net_class,
            net.freerouting.rules.ClearanceMatrix p_clearance_matrix)
    {
    ...
    private static void create_via_rule(Collection<String> p_use_via, net.freerouting.rules.NetClass p_net_class, net.freerouting.board.BasicBoard p_board, boolean p_attach_allowed)
    {
        net.freerouting.rules.ViaRule new_via_rule = new net.freerouting.rules.ViaRule(p_net_class.get_name());
        int default_via_cl_class = p_net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
    ...
    private static void create_active_trace_layers(Collection<String> p_use_layer, LayerStructure p_layer_structure, net.freerouting.rules.NetClass p_net_class)
    {
    ...
    private static void add_clearance_rule(net.freerouting.rules.ClearanceMatrix p_clearance_matrix, net.freerouting.rules.NetClass p_net_class,
     ...
     */
    static private int get_clearance_class(net.freerouting.rules.ClearanceMatrix p_clearance_matrix,
            net.freerouting.rules.NetClass p_net_class, String p_item_class_name)
    {
    ...
    private static boolean insert_logical_parts(ReadScopeParameter p_par)
    {
        ...
        {
            net.freerouting.library.Package lib_package = search_lib_package(next_part.name, p_par.logical_part_mappings, routing_board);
            if (lib_package == null)
            {
                return false;
            }
            net.freerouting.library.LogicalPart.PartPin[] board_part_pins =
                    new net.freerouting.library.LogicalPart.PartPin[next_part.part_pins.size()];
            int curr_index = 0;
            for (PartLibrary.PartPin curr_part_pin : next_part.part_pins)
            {
                ...
                board_part_pins[curr_index] =
                        new net.freerouting.library.LogicalPart.PartPin(pin_no, curr_part_pin.pin_name,
        ...
        {
            net.freerouting.library.LogicalPart curr_logical_part = routing_board.library.logical_parts.get(next_mapping.name);
            {
     ...
     */
    private static net.freerouting.library.Package search_lib_package(String p_part_name,
            java.util.Collection<PartLibrary.LogicalPartMapping> p_logical_part_mappings, net.freerouting.board.BasicBoard p_board)
    {
        for (PartLibrary.LogicalPartMapping curr_mapping : p_logical_part_mappings)
        {
            if (curr_mapping.name.equals(p_part_name))
            {
                ...
                net.freerouting.board.Component curr_component = p_board.components.get(component_name);
                if (curr_component == null)
                {
    ...
    {
        net.freerouting.board.RoutingBoard routing_board = p_par.board_handling.get_routing_board();
        net.freerouting.library.Package curr_front_package = routing_board.library.packages.get(p_lib_key, true);
        net.freerouting.library.Package curr_back_package = routing_board.library.packages.get(p_lib_key, false);
        ...
        net.freerouting.board.Component new_component = routing_board.components.add(p_location.name, component_location,
        ...
        net.freerouting.board.FixedState fixed_state;
        if (p_location.position_fixed)
        {
            fixed_state = net.freerouting.board.FixedState.SYSTEM_FIXED;
        }
        else
        {
            fixed_state = net.freerouting.board.FixedState.UNFIXED;
        }
        net.freerouting.library.Package curr_package = new_component.get_package();
        for (int i = 0; i < curr_package.pin_count(); ++i)
        {
            net.freerouting.library.Package.Pin curr_pin = curr_package.get_pin(i);
            net.freerouting.library.Padstack curr_padstack = routing_board.library.padstacks.get(curr_pin.padstack_no);
            ...
            {
                net.freerouting.rules.Net curr_board_net = routing_board.rules.nets.get(curr_pin_net.id.name, curr_pin_net.id.subnet_number);
            ...
            net.freerouting.rules.NetClass net_class;
            net.freerouting.rules.Net board_net;
            ...
            if (clearance_class < 0)
            {
                ...
                {
                    clearance_class = net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.SMD);
                }
                else
                {
                    clearance_class = net_class.default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.PIN);
                }
            }
        ...
        for (int k = 0; k <= 2; ++k)
        {
            ...
            {
                net.freerouting.library.Package.Keepout curr_keepout = keepout_arr[i];
                ...
                int clearance_class =
                        routing_board.rules.get_default_net_class().default_item_clearance_classes.get(net.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
